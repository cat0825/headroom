...
class InstallerTests(unittest.TestCase):
    def setUp(self):
        ...
        self.installer = install_hooks

    def test_every_agent_gets_a_start_and_a_prompt_entry(self):
        for agent in self.installer.SHAPES:
            # A fresh path: the developer's real Codex config may already have it.
            with patch.object(self.installer, "config_path",
                              return_value=self.root / f"{agent}.json"):
                result = self.installer.plan_agent(agent)
            self.assertTrue(result["ok"], result.get("reason"))
            self.assertEqual([action for _event, action in result["changes"]],
                             ["added", "added"])

    def test_merge_preserves_other_hooks_and_replaces_headroom(self):
        path = self.root / "settings.json"
        path.write_text(json.dumps({"hooks": {"UserPromptSubmit": [
            {"matcher": "", "hooks": [{"type": "command", "command": "other-tool"}]},
        ]}}), encoding="utf-8")
        with patch.object(self.installer, "config_path", return_value=path):
            result = self.installer.plan_agent("claude")
        entries = result["document"]["hooks"]["UserPromptSubmit"]
        self.assertEqual(len(entries), 2)
        self.assertEqual(entries[0]["hooks"][0]["command"], "other-tool")
        self.assertIn("HEADROOM_AGENT=claude", entries[1]["hooks"][0]["command"])

    def test_rerunning_updates_in_place(self):
        path = self.root / "settings.json"
        with patch.object(self.installer, "config_path", return_value=path):
            self.installer.write_config(path, self.installer.plan_agent("claude")["document"])
            second = self.installer.plan_agent("claude")
        self.assertEqual([action for _event, action in second["changes"]],
                         ["updated", "updated"])
        self.assertEqual(len(second["document"]["hooks"]["UserPromptSubmit"]), 1)
    ...
    def test_codex_keeps_its_windows_command_variant(self):
        entry = self.installer.build_entry("codex", "UserPromptSubmit", "--user-prompt")
        inner = entry["hooks"][0]
        self.assertIn("commandWindows", inner)
        self.assertTrue(inner["async"])
        self.assertEqual(entry, {"hooks": [inner]})  # UserPromptSubmit has no matcher.
        start = self.installer.build_entry("codex", "SessionStart", "--session-start")
        self.assertEqual(start["matcher"], "startup|resume|clear|compact")

    def test_gemini_entries_omit_the_matcher_key(self):
        entry = self.installer.build_entry("gemini", "SessionStart", "--session-start")
        self.assertNotIn("matcher", entry)
        self.assertNotIn("commandWindows", entry["hooks"][0])

    def test_dry_run_writes_nothing(self):
        path = self.root / "settings.json"
        with patch.object(self.installer, "config_path", return_value=path), \
                contextlib.redirect_stdout(io.StringIO()) as output:
            self.assertEqual(self.installer.main(["--agents", "claude"]), 0)
