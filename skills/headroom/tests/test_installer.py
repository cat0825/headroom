"""The hook installer: one hook script merged into each agent's own config.

Every test uses a temporary HOME, so the suite never touches a real agent's
settings. The last test drives the real hook script with each product's own
payload, which is the claim the installer exists to make: one turn, one scorer,
one ledger, whichever tool you typed into.
"""
...
class InstallerTests(unittest.TestCase):
    def setUp(self):
        ...
        # Every home override is cleared so the fake HOME decides what gets
        # installed; a developer's real CLAUDE_CONFIG_DIR must not leak in.
    ...
    def test_install_sh_wrapper_finds_python_and_passes_options_through(self):
