...
`python hooks/install_hooks.py --dry-run` prints the merge into each agent's config; running it without `--dry-run` writes it. `--agents` takes `auto` (default: Codex plus every other agent already installed here), `all`, or a comma list. It supports `codex` (`$CODEX_HOME/hooks.json`, `SessionStart` + `UserPromptSubmit`), `claude` (`$CLAUDE_CONFIG_DIR/settings.json`, the same two events), and `gemini` (`$GEMINI_DIR/settings.json`, `SessionStart` + `BeforeAgent`, millisecond timeouts). It preserves unrelated hook entries, backs up every modified file, and refuses to rewrite an unparseable config. Review and trust the new definitions in each agent before they take effect.

The hook normalizes each agent's payload — `prompt`/`user_prompt`, `session_id`/`sessionId`, `turn_id`/`promptId` — and the installer sets `HEADROOM_AGENT` explicitly so the ledger attributes the debit correctly. Codex supplies a `turn_id`; Claude Code supplies `prompt_id`, which is the same idea; Gemini CLI supplies neither, so headroom allocates a durable per-session sequence from the ledger's `hook_turns` table. Two different prompts in one session charge twice; a redelivered hook for the same turn charges once.
