...
On Windows, install the user hook with `powershell -ExecutionPolicy Bypass -File hooks/install_windows.ps1` from this skill directory. Existing `hooks.json` is never overwritten without `-Force`; merge manually if it already exists. Review and trust the two definitions in Codex `/hooks`, then start a new session. Plugin installation or implicit skill selection alone does not activate lifecycle hooks. The `SessionStart` hook launches the dashboard on loopback; `UserPromptSubmit` scores asynchronously and never blocks a response. Run `python tests/test_headroom.py` for the mechanical checks.
