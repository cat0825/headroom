...
- Optional, Windows tray card only: `python -m pip install -r skills/headroom/requirements-desktop.txt` (pystray, Pillow, pywebview) plus the Microsoft Edge WebView2 Runtime. You do not need these to run the tests.
...
- The Node test starts Python to render the dashboard; `HEADROOM_TEST_PYTHON` selects the executable (default: `python`).
...
`README.md` (English) and `README.zh-CN.md` (简体中文) should describe the same features, commands, and sections. When you change one, update the other in the same pull request. If you cannot write one of the languages, say so in the pull request so a maintainer can help.
...
- 可选，仅 Windows 托盘卡片需要：`python -m pip install -r skills/headroom/requirements-desktop.txt`（pystray、Pillow、pywebview），以及 Microsoft Edge WebView2 Runtime。运行测试不需要它们。
...
- Node 测试会调用 Python 渲染面板，可用 `HEADROOM_TEST_PYTHON` 指定 Python（默认 `python`）。
...
`README.md`（英文）和 `README.zh-CN.md`（简体中文）应描述相同的功能、命令和章节。修改其中一份时，请在同一个 PR 中同步更新另一份。如果不熟悉其中一种语言，请在 PR 中说明，维护者会协助。
